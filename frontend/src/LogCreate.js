...
function LogCreate({
  ...
  React.useEffect(() => {
    ...
    form.setFieldsValue({
      Strategy: strategies.length === 0 ? strategies[0].id : activeStrat,
    });
  }, []);
  ...
  const createLog = async (strategy, values) => {
    let ignore = false;
    values.strategy = BASE_URL + `api/strategy/${strategy}/`;
    if (!ignore) {
      // Wait for response
  ...
  const deleteLog = async (log_id) => {
    ...
    if (!ignore) {
      // Wait for response
      const response = await fetch(BASE_URL + `api/log/${log_id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("access")}`,
  ...
  const enterLoading = (log_id) => {
    ...
    setTimeout(() => {
      ...
      deleteLog(log_id).then(() => updateStates());
    }, 2000);
  };
      ...
      >
        <List
                ...
                {(fields, { add, remove }) => (
                  <>
                    {fields.map((field, index) => (
                      <Space
                        key={field.key}
                        className="d-flex flex-row align-items-center justify-content-between py-1 my-1"
                      >
                        <div className="d-flex flex-row flex-wrap flex-lg-nowrap align-items-center justify-content-start">
                          <Form.Item
                            ...
                            help={
                              logDateStatus.length - 1 >= index
                                ? "errorMsg" in logDateStatus[index]
                                  ? logDateStatus[index].errorMsg
                                  : null
                                : null
                            }
                            validateStatus={
                              logDateStatus.length - 1 >= index
                                ? "validateStatus" in logDateStatus[index]
                                  ? logDateStatus[index].validateStatus
                                  : null
                                : null
                            }
          ...
          renderItem={(item) => (
            <List.Item
              key={item.id}
              className="d-flex flex-nowrap align-items-center justify-content-between"
            >
              {item.length !== 0 ? (
                <>
                  <div className="d-flex flex-wrap align-items-center justify-content-between">
                    ...
                    <Typography.Text style={{ fontSize: 12 }}>
                      Completed {item.distance}m in {item.duration}mins.
                    </Typography.Text>
                  </div>
                  <div
