...
export const BASE_URL = "https://runninglog-app.onrender.com/"
...
function MyLayout({ currentTheme, changeTheme, configCSSManually }) {
    ...
    // Lifted so the landing page call to action can open the registration modal
    ...
    const openLoginMenu = (event) => {
        // Bootstrap closes an open dropdown as soon as a click lands outside of it, and this
        // call to action sits outside of the header dropdown. Keeping the click away from the
        // document listener lets the login form stay open, while the Bootstrap API keeps the
        // dropdown state (aria-expanded, popper, and the show class) in sync.
        if (event && typeof event.stopPropagation === "function") {
            event.stopPropagation()
        }
        const loginMenuButton = document.getElementById("dropdownLoginMenuButton1")
        if (loginMenuButton) {
            BootstrapDropdown.getOrCreateInstance(loginMenuButton).show()
        }
    }
    ...
    React.useEffect(() => {
        ...
        if (
            fetchInfo.strategy.errorMsg.detail !== undefined ||
            fetchInfo.log.errorMsg.detail !== undefined ||
            fetchInfo.lap.errorMsg.detail !== undefined
        ) {
            logOut()
    ...
    const getLaps = async () => {
        ...
        if (!ignore) {
            // Wait for response
            const response = await fetch(BASE_URL + `api/lap?strategy=${activeStrat}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
    ...
    const getLogs = async () => {
        ...
        if (!ignore) {
            // Wait for response
            const response = await fetch(BASE_URL + `api/log?strategy=${activeStrat}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${localStorage.getItem("access")}`,
    ...
    return (
        <Layout>
            ...
            >
                ...
                {/* The app menu only applies to a logged in user */}
            ...
            <Layout>
                    ...
                    <div
                        className={
                            loginStatus.status === true
                                ? "d-flex flex-row justify-content-between align-items-center pt-3"
                            ...
                            loginStatus.status === false ? (
                                <div className="dropdown dropdown-login me-4">
                                    ...
                                    >
                                        ...
                                        {loginStatus.status === false ? "Login" : localStorage.username}
                                    </button>
                                    ...
                                    {/*validate lap duration(using max)*/}
                                    {
                                        ...
                                        validCodeReg.test(fetchInfo.strategy.state) ? (
                                            menu === "1" ? (
                                                <>
                                                    <div className="mb-3">
                                                        <Select
                                                            ...
                                                            defaultValue={activeStrat === null ? null : activeStrat}
                                                        >
                                                            {strategies.length !== 0
                                                ...
                                                validCodeReg.test(fetchInfo.log.state) && activeStrat !== null ? (
                                                    <>
                                                        <Select
                                                            disabled={!strategies.length === 0}
                                                            style={{ width: 150 }}
                                                            onChange={handleStratChange}
                                                            defaultValue={strategies.length === 0 ? strategies[0].id : activeStrat}
                                                        >
                                                            {strategies.map((strategy) => (
                                                                <Option key={strategy.id} value={strategy.id}>
                                                                    {strategy.name}
                                        ...
                                        invalidServerCodeReg.test(fetchInfo.strategy.state) ||
                                          invalidServerCodeReg.test(fetchInfo.log.state) ||
                                          invalidServerCodeReg.test(fetchInfo.lap.state) ? (
                                            <Result status="warning" title="There are some problems on the server-side." />
                                        ) : (
                                            <Result status="warning" title="There are some problems with your internet connection." />
                                        )
                                    }
                                </>
                            ) : (
                                <>
                                    <LandingPage onSignUp={() => setRegisterOpen(true)} onSignIn={openLoginMenu} />
                                    <RegisterForm logIn={logIn} open={registerOpen} setOpen={setRegisterOpen} showTrigger={false} />
                                </>
                            )
                        }
                    </div>
                ...
                <Footer style={{ textAlign: "center" }}>
                    Running Log ©{new Date().getFullYear()} Created by <a href="https://github.com/laulj/">laulj</a>
                </Footer>
            </Layout>
        </Layout>
    )
