...
function MyLayout({ currentTheme, changeTheme, configCSSManually }) {
  ...
  const getLaps = async () => {
    ...
    if (!ignore) {
      ...
      const response = await fetch(
        BASE_URL + `api/lap?strategy=${activeStrat}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const getLogs = async () => {
    ...
    if (!ignore) {
      // Wait for response
      const response = await fetch(
        BASE_URL + `api/log?strategy=${activeStrat}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    <Layout>
      ...
      <Layout>
            ...
            }
            {
              ...
              loginStatus.status === false ? (
                <div className="dropdown dropdown-login me-4">
                  ...
                  <ul
                    className="dropdown-menu dropdown-login-menu px-2"
                    aria-labelledby="dropdownLoginMenuButton1"
                  >
                  ...
                  {/*validate lap duration(using max)*/}
                  {
                    ...
                    validCodeReg.test(fetchInfo.strategy.state) ? (
                      menu === "1" ? (
                        <>
                          <div className="mb-3">
                            <Select
                              ...
                              defaultValue={
                                activeStrat === null ? null : activeStrat
                              }
                            >
                              {strategies.length !== 0
                        ...
                        validCodeReg.test(fetchInfo.log.state) &&
                        activeStrat !== null ? (
                          <>
                            <Select
                              disabled={!strategies.length === 0}
                          ...
                          <Result
                            status="info"
                            title="Add some strategies and logs before the data can be plotted."
                          />
                        )
                      ) : menu === "3" ? (
                        <>
                    ...
                    invalidServerCodeReg.test(fetchInfo.strategy.state) ||
                      ...
                      <Result
                        status="warning"
                        title="There are some problems on the server-side."
                      />
                    ) : (
                      <Result
                        status="warning"
        ...
        <Footer style={{ textAlign: "center" }}>
          Running Log ©2023 Created by{" "}
          <a href="https://www.instagram.com/lokjing/">laulj</a>
        </Footer>
      </Layout>
    </Layout>
  );
