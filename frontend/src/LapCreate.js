...
function LapCreate({ logs, activeLog, setActiveLog, laps, updateStates }) {
  ...
  const getHighestLapOrder = () => {
    if (laps.filter((lap) => lap.log === activeLog.id).length !== 0) {
      const highest_order = laps
        .filter((lap) => lap.log === activeLog.id)
        .sort((a, b) => {
  ...
  const createLap = async (log, values) => {
    ...
    if (!ignore) {
      // Wait for response
      const response = await fetch(BASE_URL + "api/lap/", {
        method: "POST",
        headers: {
  ...
  const deleteLap = async (lap_id) => {
    ...
    if (!ignore) {
      // Wait for response
      const response = await fetch(BASE_URL + `api/lap/${lap_id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("access")}`,
  ...
  const enterLoading = (lap_id) => {
    ...
    setTimeout(() => {
      ...
      deleteLap(lap_id).then(() => updateStates());
    }, 2000);
  };
                  ...
                  <>
                    <div className="lapAddFields">
                      {fields.map((field, index) => (
                        <Space
                          key={field.key}
                          className="d-flex flex-row align-items-center justify-content-start py-1 my-1"
                        >
                          ...
                          <MinusCircleOutlined
                            key={index}
                            ref={(el) => (minusRef.current[index] = el)}
